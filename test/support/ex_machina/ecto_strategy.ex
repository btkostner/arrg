defmodule Arrg.ExMachina.EctoStrategy do
  @moduledoc """
  An Ecto strategy for ExMachina that supports PolymorphicEmbed.

  See this GitHub issue for more details as well as original code source:
  https://github.com/thoughtbot/ex_machina/issues/407
  """
  ...
  defp cast_polymorphic_embeds(%{__struct__: schema} = struct) do
    ...
    |> Enum.reduce(struct, fn embed_key, struct ->
      casted_value = Map.get(struct, embed_key)
